...
"""
        ...
        agent_layout.addWidget(QLabel("Sleep Between Runs (minutes):"), 5, 0)
        self.sleep_between_runs = QSpinBox()
        self.sleep_between_runs.setRange(1, 120)
        self.sleep_between_runs.setValue(5)  # Default COPYBOT_INTERVAL_MINUTES
        self.sleep_between_runs.setToolTip("How often to run the CopyBot agent")
        agent_layout.addWidget(self.sleep_between_runs, 5, 1)
    ...
    def save_config(self):
        ...
        try:
            ...
            config_content = self.update_config_value(config_content, "TRADING_MODE", f'"{"leverage" if self.leverage_mode.isChecked() else "spot"}"')
            config_content = self.update_config_value(config_content, "USE_HYPERLIQUID", str(self.leverage_mode.isChecked()))
            ...
            # 3. Excluded tokens
            excluded_text = self.excluded_tokens.toPlainText()
            excluded_list = [line.strip() for line in excluded_text.strip().split('\n') if line.strip()]
            
            # Add additional excluded tokens if any
            ...
            if additional_text.strip():
                ...
                excluded_list.extend(additional_list)
            
            # Format the excluded tokens list
            excluded_str = "[USDC_ADDRESS, SOL_ADDRESS"
            for token in excluded_list:
                if token.strip() and token != "USDC_ADDRESS" and token != "SOL_ADDRESS":
            ...
            # Update Hyperliquid mappings
            mapping_text = self.token_hl_mapping.toPlainText()
            mapping_lines = [line.strip() for line in mapping_text.strip().split('\n') if line.strip()]
            ...
            config_content = self.update_config_value(config_content, "TOKEN_TO_HL_MAPPING", mapping_str, multiline=True)
            ...
            try:
                ...
                # Update the copybot interval in ACTIVE_AGENTS
                ...
                print(f"Warning: Could not update interval in main.py: {str(e)}")
    ...
    def refresh_charts(self):
        ...
        for csv_file in csv_files:
            try:
                ...
                # Analysis details with new fields
                details = QLabel(
                    f"Timeframe: {latest['timeframe']}\n"
                    f"Signal: {latest['signal']}\n"
                    f"Confidence: {latest['confidence']}%\n"
                    f"Market Direction: {latest['direction']}\n"
                    f"Market Regime: {latest['market_regime']}\n"
                    f"Volume Trend: {latest['volume_trend']}\n"
                    f"Entry Price: ${latest['price']:.6f}\n"
